...
class Network(nn.Module):
    def __init__(self, input_size, nb_action):
        ...
        # experimentation
        self.fc1 = nn.Linear(input_size, 100)  # Full connection 1
        self.fc2 = nn.Linear(100, nb_action)  # Full connection 2
...
class ReplayMemory(object):
    ...
    def update_last_samples(self, actions_count, reward):
        for index in range(len(self.memory)-actions_count, len(self.memory)):
            last_state, new_state, last_action, last_reward = self.memory[index]
            new_reward = torch.Tensor([reward]) if last_reward >=0 else last_reward
            self.memory[index] = (
                last_state, new_state, last_action, new_reward
            )
...
class Dqn():
    ...
    def select_action(self, state):
        ...
        probs = F.softmax(self.model(Variable(state, volatile=True)) * 30)  # T=100
