...
class Game(Widget):
    ...
    def update(self, dt):
        ...
        #last_signal = [distance/1400, self.car.signal1, self.car.signal2, self.car.signal3, orientation, -orientation]
        ...
        if sand[int(self.car.x), int(self.car.y)] > 0:
            ...
            last_reward = 0.1 if distance < last_distance else -0.05
        ...
        if self.car.y > self.height - 10:
            ...
            last_reward = -1
        """
        print(f"Distance: {distance/1400} | Reward: {last_reward} | "
              f"Orientation: {orientation} | epoch: {self.epoch} | "
        ...
        if distance < 100:
            print(f"Distance: {distance/1400} | Reward: {last_reward} | "
              f"Orientation: {orientation} | epoch: {self.epoch} | "
              f"actions: {self.actions_count} | Reward: {(1/self.actions_count)*10}")
            goal_x = self.width - goal_x
            goal_y = self.height - goal_y
            self.epoch += 1
            if self.epoch > 5:
                self.brain.memory.update_last_samples(self.actions_count, (1/self.actions_count)*10)
                #self.brain.learn()
            self.actions_count = 0
